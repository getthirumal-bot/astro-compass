...
class QuotaChecker:
    ...
    def __init__(self, quota_file='data/daily_quota.json'):
        ...
        self.FREE_LIFETIME_LIMIT = 7  # Updated to 7 questions
    ...
    def can_user_ask(self, user: Dict) -> Dict:
        ...
        # Step 1: Check if paid subscriber (BASIC, FAMILY, VIP all have unlimited)
        if user.get('subscription', 'FREE') in ['BASIC', 'FAMILY', 'VIP', 'PAID', 'PREMIUM']:
            return {
                'allowed': True,
                'api_tier': 'paid',
        ...
        lifetime_questions = user.get('lifetime_questions', 0)  # FIX: Use .get() for new users
        
        if lifetime_questions >= self.FREE_LIFETIME_LIMIT:
            return {
                'allowed': False,
                'reason': 'lifetime_limit',
                'message': (
                    f"🎯 You've used all {self.FREE_LIFETIME_LIMIT} free questions.\n\n"
                    f"Support our mission - choose your plan:\n"
                    f"✓ BASIC (₹99/$2) - Unlimited questions, 1 chart\n"
                    f"✓ FAMILY (₹499/$8) - Unlimited questions, 8 charts\n"
                    f"✓ VIP (₹4,000/$40) - Everything unlimited\n\n"
                    f"[Upgrade Now]"
                )
            }
        ...
        if quota_data['free_queries_today'] >= self.FREE_DAILY_TOKEN_LIMIT:
            ...
            return {
                ...
                'message': (
                    ...
                    f"Your questions: {lifetime_questions}/{self.FREE_LIFETIME_LIMIT}\n\n"
                    f"Can't wait? Upgrade for unlimited!\n\n"
                    f"[Upgrade Now] [Ask Tomorrow]"
                )
            }

        # Step 4: All checks passed - allow free query
        remaining = self.FREE_LIFETIME_LIMIT - lifetime_questions
        quota_data = self._get_quota_data()
        reset_time = datetime.fromisoformat(quota_data['reset_time'])
        ...
        return {
            ...
            'api_tier': 'free',
            'message': (
                f"✓ Question {lifetime_questions + 1}/{self.FREE_LIFETIME_LIMIT}\n"
    ...
    def get_usage_stats(self, user: Dict) -> str:
        ...
        quota_data = self._get_quota_data()
        lifetime_questions = user.get('lifetime_questions', 0)  # FIX: Use .get()

        stats = f"""
📊 YOUR USAGE STATS:
════════════════════════════════
Subscription: {user.get('subscription', 'FREE')}
Lifetime Questions: {lifetime_questions}/{self.FREE_LIFETIME_LIMIT}
...
if __name__ == "__main__":
    ...
    # Test with paid user
    print("\n[Test 3] Simulating BASIC user...")
    db.update_user(test_phone, {'subscription': 'BASIC'})
    user = db.get_user(test_phone)
